...
module PuppetBuildUtil
  ...
  class ModuleMetaData
    ...
    def intialize(meta_json, config=Configuration.new)
    ...
    def write(meta_json)
      File.open(meta_json, 'w') do |f|
        f.write(JSON.pretty_generate(@metadata)) unless @metadata
      end
    end
    ...
    def bump(part)
      ...
      new_version="#{version_spec}-#{v_specs[4]}"
    ...
    def retrive_version()
      ...
      version_part.split('.') << release_part
    end
